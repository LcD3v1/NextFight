...
async def test_hybrid_state_prediction_timeline_and_idempotency(
    ...
    async with factory() as session:
        event_id, first_id, second_id = await _seed_card(session)
...
async def _seed_card(session: AsyncSession) -> tuple[UUID, UUID, UUID]:
    suffix = uuid4().hex
    ...
    return event.id, first.id, second.id


async def _cleanup(session: AsyncSession, event_id: UUID) -> None:
    fights = list(
